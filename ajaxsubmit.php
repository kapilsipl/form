...
else if(isset($_FILES["filege"]["type"]))
 {
	if ($_FILES['filege']['size'] != 0) {
				
				$sourcePath = $_FILES['filege']['tmp_name']; // Storing source path of the file in a variable
				//$targetPath = "upload/".$_FILES['filege']['name']; // Target path where file is to be stored
				$targetPath = 'upload/' . date('Y-m-d-H-i-s') . '_' . uniqid() .$_FILES['filege']['name'];
				move_uploaded_file($sourcePath,$targetPath) ; // Moving Uploaded file
		}		
